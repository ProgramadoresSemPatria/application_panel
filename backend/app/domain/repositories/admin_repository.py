...
class AdminRepository:
    ...
    async def get_user_rows(
        ...
        if seniority:
            stmt = stmt.where(UserModel.seniority_level == seniority)
    ...
    async def get_user_growth(self, months: int = 12) -> list[dict]:
        ...
        stmt = (
            select(
                func.date_trunc('month', UserModel.created_at).label('month'),
    ...
    async def get_top_companies(self, limit: int = 10) -> list[dict]:
        stmt = (
            select(
                CompanyModel.name,
                func.count(ApplicationModel.id).label('total_across_users'),
    ...
    async def get_admin_companies(
        ...
        stmt = (
            ...
            .outerjoin(UserModel, CompanyModel.created_by == UserModel.id)
        )
    ...
    async def count_entity_references(
        ...
        if entity_type == 'platform':
            ...
            stmt = select(func.count(ApplicationStepModel.id)).where(
                ApplicationStepModel.step_id == entity_id
            )
