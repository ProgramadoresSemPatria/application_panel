...
class GetPlatformStatsUseCase:
    ...
    async def execute(self, user_id: int, cycle_id: int | None = None) -> list[PlarformAppDTO]:
        a_platform = await self.user_stats_repo.count_applications_grouped_by_platform(
            user_id, cycle_id
        )
