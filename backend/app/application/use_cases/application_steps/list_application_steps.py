...
class ListApplicationStepsUseCase:
    ...
    async def execute(self, application_id: int, user_id: int) -> list[ApplicationStepDTO]:
        application = await self.app_repo.get_by_id_and_user_id(application_id, user_id)
        if not application:
            raise ResourceNotFound('Application not found or not owned by user')

        steps = await self.app_step_repo.get_all_by_application_id(application_id)
