...
class UpdateApplicationUseCase:
    ...
    async def execute(self, id: int, data: ApplicationUpdateDTO) -> ApplicationDTO:
        ensure_not_in_future(data.application_date, 'application_date')

        application = await self.application_repo.get_by_id_and_user_id(id, data.user_id)
        if not application:
        ...
        application.link_to_job = str(data.link_to_job) if data.link_to_job else None
