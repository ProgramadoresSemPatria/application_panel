    ...
    async def execute(self, feedback_id: int, admin_id: int) -> None:
        ...
        if refs > 0:
            logger.warning(
                ...
                extra={
                    'extra_data': {
                        'event': ('admin_delete_feedback_definition_blocked'),
            ...
            raise ResourceConflict(
                f'Cannot delete: {refs} application(s) reference this feedback definition'
            )

        await self.feedback_def_repo.delete(feedback_id)

        logger.info(
            f'Admin deleted feedback definition: {feedback_id}',
            extra={
                'extra_data': {
                    'event': ('admin_delete_feedback_definition'),
                    'feedback_definition_id': feedback_id,
                    'admin_id': admin_id,
                }
            },
